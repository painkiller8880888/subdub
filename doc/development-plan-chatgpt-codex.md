...
`assets`に用意された2人分のキャラクターデータを調査し、`character-mentor`と`character-learner`へ対応付ける。実在する通常会話・指差し状態の会話・非会話状態、会話ポーズの口閉じ・口開き、同一キャンバス、固定位置、透過PNGを検証する。台本上の4表情語彙へ素材を推測で割り当てない。
