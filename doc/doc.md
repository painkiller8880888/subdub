...
構造化したスライドや図解を AI または自前スキルで生成する方式は採用しない。制作前に現場動画、写真、帳票スキャンを素材ライブラリへ登録し、台本編集画面から検索・選択して割り当てる。
...
1. 各セリフカードから素材ピッカーを開き、キーワード、タグ、素材種別、部門、対象システム、利用状態で絞り込む。
...
6. 同じ台本範囲への割り当て変更、解除、前後の範囲への延長・短縮を行えるようにする。
...
- ビジュアル素材を大きく表示する場面ではキャラクターを縮小または非表示にできる。
...
WebUI は単一ユーザーがローカル環境で使用し、制作パイプラインの段階ごとに同じプロジェクトを編集する。まず 6.1 の入力作成と 6.2 の構成案生成・レビュー、続いて 6.3 の人力台本作成・レビュー、6.4 の素材登録・検索・割り当てを実装する。
...
素材ピッカーは台本編集画面上のドロワーまたはモーダルとして開く。上部にキーワード検索とタグフィルター、中央にサムネイル一覧、詳細領域にプレビューとメタデータを表示する。AI サジェストを実行した場合は、検索に使われた必須タグ、任意タグ、除外タグ、候補理由を表示し、人間がタグを修正して再検索できるようにする。

素材を選択した後、適用する開始・終了セリフと種別固有の表示設定を編集する。割り当て済み素材はセリフカード内に表示し、クリックで差し替え、範囲変更、解除を行えるようにする。未割り当て区間と参照エラーは台本全体の進捗として確認できるようにする。
