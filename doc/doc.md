...
Issue #147（SW-00）は docs-only の仕様改訂である。`doc/doc.md` と `implementation-spec.md` だけを更新し、コード、Zod schema、migration、API、React UI、compiler、Remotion、テストコードは変更しない。現在の実装基準は `VideoProject 1.3.0` / `RenderManifest 2.4.0` とする。

プロジェクト固有の ScreenTemplate 選択は `ScriptSection.screenTemplateId` だけを正本とする。`ScriptLine` には `screenTemplateId` を持たせず、section 内の全 line が section の template を使用する。section の明示参照が missing / inactive でも別 template へ自動代替せず、section header から active template を選び直す修正対象として表示し、出力 validation では error とする。

`/projects/{projectId}/script` は、情報密度を上げた compact line card を標準とする。通常表示は本文 3 行と操作 1 行の計 4 行で構成する。1 行目は line ID、speaker selector、character physical variant、音声再生、音声再生成、音声調整を置く。2 行目は `subtitleText`、3 行目は読み上げ用の `spokenText` / よみがなを置き、両方とも通常時は compact な 1 行表示とする。4 行目は上へ移動、下へ移動、複製、削除を置く。本文の選択・編集時だけ入力領域を expand し、編集終了後は compact 表示へ戻す。音声調整の詳細は card 内へ常時展開せず modal / dialog に置く。

line card の左側には、全 line のフル 16:9 preview を常時表示しない。section の先頭 line、section 境界で template または background が変わる line、generic visual の persistent canvas state（show / hide / play / pause / resume / end など）が変化する line、表示設定がその境界から変化する line では full screen preview を表示する。それ以外の line は dialogue / subtitle 領域だけを確認できる compact preview とする。`subtitleText`、`spokenText` / よみがな、speaker、character physical variant、voice parameter、音声の current / stale state だけの変化は full preview の trigger にしない。

preview の表示モードは React component 内の前後比較へ閉じ込めず、`persistentScreenState(line N - 1) != persistentScreenState(line N)` を決定論的に評価する pure helper / read model として定義する。full preview と compact preview は同じ ScreenTemplate resolver / layout component の解決結果を使い、compact preview 専用の geometry や CSS 座標を再実装しない。generic `VisualAssignment` 自体は維持し、line template override 廃止を理由に section 内の template 境界で segment 化しない。表示素材 cue の state boundary による segment 化は、後続の cue Issue で別責務として定義する。

後続実装の境界は SW-01 で `VideoProject 1.3.0 → 1.4.0` の line override removal / migration、SW-02 で compact ScriptPage・section-only selector・voice adjustment modal、SW-03 で change-only full preview / dialogue-only compact preview を扱い、その後に表示素材 cue を統合する。1.4.0 migration は section の `screenTemplateId` を authority として維持し、section の分割や多数決による template 変更を行わない。既存 line override を削除する場合は、`lineId`、旧 template ID、section template ID、`migrationId` を project migration log へ記録する。
...
- project-specific な template selection は `project.json` の section にだけ保存する。section の `screenTemplateId` が同じ section 内の全 line の template authority となり、missing / inactive な明示参照を別 template へ自動代替しない。
...
- section ごとの `screenTemplateId`。line には ScreenTemplate の選択参照を保存しない。
- 画面表示用の字幕テキスト
...
現行 `VideoProject 1.3.0` では、`script.sections[]` に `screenTemplateId: string` を保存し、`ScriptLine` には ScreenTemplate の選択参照を保存しない。section の `screenTemplateId` が、その section に属する全 line の唯一の project-specific template authority である。新規 section は `screen-template-standard` または人間が選択した active template を持つ。

解決規則は次の 1 つだけとする。

```text
section.screenTemplateId
  → section 内の全 line に適用する ScreenTemplate
```

section header から明示された template が missing / inactive の場合は、編集中に validation と修正導線を表示し、別 template へ自動代替しない。出力 validation では error とし、未解決の layout を持つ manifest を生成しない。workspace SQLite には project / section の適用関係を保存せず、project JSON の section 参照だけを正本とする。

後続の `VideoProject 1.3.0 → 1.4.0` migration では、既存 line に残る non-null override を削除する。section を分割したり、line override の多数決で section template を変更したりせず、section の `screenTemplateId` を authority として維持する。削除した override は `lineId`、old template ID、section template ID、`migrationId` を project migration log に記録する。

line override がなくなるため、同一 section 内の template 境界だけを理由に `VisualAssignment` を `RenderVisualV24` segment へ分割しない。generic visual の persistent canvas state による segment や表示素材 cue の境界は、後続の cue model の責務として定義する。section 境界による layout / visual state の変化は、section 単位の resolved layout として扱う。
...
現行の `RenderManifest 2.4.0` は、compile 時に検証した section template snapshot と、その snapshot から得た layout を section 単位で固定する。template ID だけでなく、`sectionTitle`、template revision、deterministic `templateHash`、1920 × 1080 canvas、各 element の rect / rotation、dialogue / section title の font size、character の `characterId` / `flipX`、primary content slot を `sectionLayouts[]` に保存する。line は `sectionId` から親 section の resolved layout を参照し、line-level template ID / override は持たない。compiler と Remotion は DB を再参照せず、この解決済み layout を描画する。

line override 廃止だけを理由に `RenderManifest 2.4.0` を 2.5.0 へ version bump しない。line-level template ID、override 専用の validation、line template boundary 専用の visual segment partition は現行仕様から除外し、section layout と後続 cue model の責務だけを manifest へ固定する。
...
1 つの `VisualAssignment` が複数 line または複数 section にまたがる場合、`RenderVisualV24` の分割単位は section 境界または persistent canvas state / 後続 cue model の明示的な境界とする。同一 section 内で line ごとに template を比較して segment 化しない。各 segment は `sourceAssignmentId`、決定論的な segment ID、segment 順序、開始・終了 line ID、対象 section の template snapshot、`from` / `durationInFrames`、resolved display を持つ。segment ID と partition は `compilerInputHash` に含める。
...
│  └─ sectionId（親 section layout を参照）
├─ visuals[]
│  ├─ id（決定論的 segment ID）
│  ├─ sourceAssignmentId / segmentIndex
│  ├─ segmentStartLineId / segmentEndLineId
│  ├─ sectionId / templateRevision / templateHash
...
template の revision / hash、section title、project の section selection、ScreenTemplate の element geometry、speaker mapping、generic assignment の inner transform、VisualAssignment の section / cue segment partition（source assignment ID、segment line 境界、section template ID / revision / hash、segment の `from` / `durationInFrames`）、resolved generic visual の `outerFrame` / `contentClip` / `fit` / `crop` / annotation、動画 segment の provenance `startMs` / `endMs` と authoritative `sourceTrimBeforeFrame` / `sourceTrimAfterFrame`、`displayCoordinateSpace`、`prioritizeVisual` の適用結果のいずれかが変わった場合は `compilerInputHash` を変え、旧 manifest を current とみなさない。`displayCoordinateSpace` は compiler input として legacy adapter の選択に使うが、V24 の resolved visual display へ raw 値を残して Remotion に再解釈させない。過去 revision の template を project.json に埋め込む snapshot history や rollback UI は今回対象外とする。
...
7. section header で section 全体の `screenTemplateId` を選択する。line card には template selector、inherit badge、「セクション設定に戻す」を置かず、section の template を全 line に適用する。適用後の preview は state change のある line では full screen、通常 line では dialogue / subtitle 領域だけの compact preview とする。
...
14. section の ScreenTemplate と `ScriptSection.name` からの `sectionTitle`、`speaker-1` / `speaker-2` の character mapping、resolved geometry、transform、font size、`flipX`、content slot、segment ごとの generic visual の `outerFrame` / `contentClip` / `fit` / `crop` / annotation を共有 resolver で確定し、現行 `RenderManifest 2.4.0` の section layout、line、visual、全体 duration を生成する。line は親 section の resolved layout を参照し、line-level template ID を保存しない。動画 segment の provenance `startMs` / `endMs` は元 assignment range を保持し、assignment 開始からの経過時間を反映した authoritative `sourceTrimBeforeFrame` / `sourceTrimAfterFrame` を解決する。
...
- WebUI の line-card preview と MP4 レンダリングには、同じ timeline compiler、ScreenTemplate geometry resolver、同じ layout component、同じ resolved `RenderManifest 2.4.0` を使用する。preview 専用の固定 CSS 座標や resolver を作らない。
...
- section の `screenTemplateId` を、その section に属する全 line の template authority として解決する。line には template selector または template ID を保存しない。
...
- `1.3.0 → 1.4.0` migration が section の `screenTemplateId` を authority として維持し、line override を削除し、`lineId`、old template ID、section template ID、`migrationId` を migration log へ記録することを検証する。section の分割、template の多数決変更、missing / inactive template の自動代替を行わない。
...
- `VideoProject 1.3.0` の section `screenTemplateId` を `project.json` へ保存し、section 内の全 line へ同じ template を適用できる。line-level ScreenTemplate selector / override は持たない。
...
- `RenderManifest 2.4.0` に section ごとの `sectionTitle`、resolved layout、template revision / hash、font size、`flipX`、content slot、generic visual の `RenderVisualV24.display` を固定し、line は parent section layout を参照して Remotion が SQLite や raw display coordinate space を直接参照せずに描画できる。
- 2 キャラクターの掛け合いを表現できる。
...
- `RenderManifest 2.4.0` は section template の revision / deterministic hash と resolved layout を compiler input に含め、line は parent section layout を参照する。line template override を理由に visual segment を増やさず、section 境界または後続 cue boundary だけを segment の境界とする。
...
- `project.json` には ScreenTemplate の catalog や preview 素材を埋め込まず、`script.sections[].screenTemplateId` の project-specific な選択参照だけを保存する。editor の実素材選択は一時 UI state とし、`visualId`、`variantId`、`assetId` を template 本体へ書き込まない。
...
Issue #147（SW-00）は `doc/doc.md` と `implementation-spec.md` だけを更新する docs-only の仕様改訂である。コード、schema、migration、API、React UI、compiler、Remotion、テストコードは後続 Issue で実装する。現在の正本は section-only ScreenTemplate、compact line card、persistent canvas state に基づく preview mode 判定とする。
