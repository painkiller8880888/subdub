...
- `edge` は `before` / `after`、`action` は `pause` / `resume` に限定する。`pause` は playing state、`resume` は paused state でだけ有効とし、同じ line / edge の相反 cue を重複保存しない。
...
type RenderVisualV25 = Omit<RenderVisualV24, "screenTemplateId" | "display"> & {
  // The visual segment resolves the section template through sectionId.
  sectionId: string;
  display:
    | ResolvedVideoDisplayV25
    | ResolvedImageDisplay
    | ResolvedDocumentDisplay;
};
...
- VP-01 の `VideoDisplayV15.playbackCues` が video kind にだけ存在し、cue `lineId` が同一 section の assignment range 内、`edge` が before / after、`action` が pause / resume であること。range 外、state 不一致、相反重複、photo / `document_scan` cue、initial / final の synthetic cue を拒否すること
