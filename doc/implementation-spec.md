...
2. `/replace` の request に含まれる `expectedRevision` と現在の Asset revision を比較する。一致した同じ write transaction で次の version number を確保し、AssetVersion row を `processing` で insert する。この row に、受付時点の Asset revision を `baseRevision`、想定した旧 currentVersion を `baseCurrentVersion`、`StagedUploadRecord.stagingRelativePath` を `stagingPath` として保存する。`stagingPath` は例えば `staging/{uploadId}/upload.bin` の staging root 相対値とし、worker の再起動後に `(assetId, version)` から復元できるようにする。旧 current version の media と thumbnail は変更しない。
...
- `stagingPath` が `StagedUploadRecord.stagingRelativePath` から AssetVersion へ永続化され、`staging/{uploadId}/upload.bin` のような staging root 相対 locator を worker 再起動後に解決できること。success commit 後だけ staging cleanup すること
