...
describe("OutlineGenerationService", () => {
  ...
  it("regenerates a stale outline and replaces it after AI succeeds", async () => {
    const { workspaceRoot, repository, project } = await setupProject();
    roots.push(workspaceRoot);
    const createId = vi
      .fn()
      .mockReturnValueOnce("run-outline-first")
      .mockReturnValueOnce("run-outline-regenerated");
    const service = new OutlineGenerationService({
      repository,
      modelService: {
        listModels: async () => ({
    ...
    const source = await repository.readSource(project.metadata.id);
    const stale = await repository.saveSource(
      project.metadata.id,
      `${source.markdown}\n\n追記`,
      first.revision
    );
    const regenerated = await service.regenerate(project.metadata.id, {
      expectedRevision: stale.revision
    });

    expect(regenerated.revision).toBe(stale.revision + 1);
    expect(regenerated.outline.generationRunId).toBe("run-outline-regenerated");
    expect(regenerated.outline.sourceHash).toBe(regenerated.source.sha256);
    expect(regenerated.outline.sourceHash).not.toBe(first.outline.sourceHash);
  });

  it("keeps the stale outline when regeneration fails before replacement", async () => {
    const { workspaceRoot, repository, project } = await setupProject();
    roots.push(workspaceRoot);
    const firstService = new OutlineGenerationService({
      repository,
      modelService: {
        listModels: async () => ({
          models: [model()],
      ...
      chatAdapter: fakeChat(candidate()),
      now: () => NOW,
      createId: () => "run-outline-first"
    });
    const first = await firstService.generate(project.metadata.id, {
      expectedRevision: project.revision
    });
    const source = await repository.readSource(project.metadata.id);
    const stale = await repository.saveSource(
    ...
    const failingService = new OutlineGenerationService({
      repository,
      modelService: {
        listModels: async () => ({
          models: [model()],
      ...
      chatAdapter: fakeChat({}),
      now: () => NOW,
      createId: () => "run-outline-failed-regeneration"
    });

    await expect(
      failingService.regenerate(project.metadata.id, {
        expectedRevision: stale.revision
      })
    ).rejects.toMatchObject({ code: "OUTLINE_GENERATION_SCHEMA_INVALID" });
    const unchanged = await repository.read(project.metadata.id);
    expect(unchanged.revision).toBe(stale.revision);
    expect(unchanged.outline.sections).toEqual(first.outline.sections);
    expect(unchanged.outline.generationRunId).toBe(
      first.outline.generationRunId
    );
    expect(unchanged.outline.sourceHash).toBe(first.outline.sourceHash);
  });
